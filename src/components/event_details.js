const event_details = {
  grooveArena: {
    start: "12:30 PM",
    end: "6:00 PM",
    location: "Main Bldg Quadrangle",
    category: "Dance (GROOVE ARENA)",
    day: "Jan 29",
    teamSize: 19,
    imageUrl:
      "https://media.istockphoto.com/id/1257938447/vector/musical-summer-dance-festival-vector-illustration-of-musicians-dancers-disco-dancing-people.jpg?s=1024x1024&w=is&k=20&c=xmrxYB18DSyXYtqkwEXA6U1je0eKQRt6VOU3D9SGNQg=",
  },
  soloSynergy: {
    start: "11:00 AM",
    end: "2:00 PM",
    location: "Stage 2",
    category: "Dance (GROOVE ARENA)",
    day: "Jan 31",
    teamSize: 1,
    Participation: "1 participant per class",
    imageUrl:
      "https://media.istockphoto.com/id/858777608/vector/dance-battle-party-poster.jpg?s=612x612&w=0&k=20&c=Adk3VCE61K6aQ4SsE_EMOImIY_ByTkhY_WZps8c2s_U=",
  },
  nrutyanjali: {
    start: "10:00 AM",
    end: "1:00 PM",
    location: "Main Bldg Quadrangle",
    category: "Dance (GROOVE ARENA)",
    day: "Feb 1",
    teamSize: 5,
    Participation: "1 team per class",
    imageUrl:
      "https://www.holidify.com/images/cmsuploads/compressed/27858916_1744144345636773_5809530212233948261_n_20180410145639.jpg",
  },
  streetBrawl: {
    start: "3:00 PM",
    end: "5:00 PM",
    location: "Stage 2",
    category: "Dance (GROOVE ARENA)",
    day: "Jan 30",
    teamSize: 1,
    Participation: "1 participant per class",
    imageUrl:
      "https://media.istockphoto.com/id/971376146/vector/dance-off.jpg?s=612x612&w=0&k=20&c=6bgzDGsIG5L06ShVUBzWbHtQoI5x3Sk8H67B9aXciCE=",
  },
  stepItUp: {
    start: "10:00 AM",
    end: "12:30 PM",
    location: "Main Bldg Quadrangle",
    category: "Dance (GROOVE ARENA)",
    day: "Jan 30",
    teamSize: "5-15 members + 1-2 helpers(only if needed)",
    Participation: "Multiple teams from same college",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
   "Sargam Symphony": {
    start: "10:00 AM",
    end: "1:30 PM",
    location: "Main Bldg Quadrangle",
    category: "Music (JAMSPHERE)",
    day: "Jan 31",
    teamSize: 2,
    Participation: "1 team per class",
    imageUrl:
       "https://media.licdn.com/dms/image/v2/D5612AQFRw-MO07-Y4g/article-cover_image-shrink_720_1280/article-cover_image-shrink_720_1280/0/1710541180651?e=1741824000&v=beta&t=vwx7Ae6WKbzg-bkkh4uP4rGpjQBqmi-3KBL2EkPMT1M",
  },
   "Suron ke Sikandar": {
    start: "10:00 AM",
    end: "12:00 PM",
    location: "KB Joshi Audi",
    category: "Fun (GATHERING HUB)",
    day: "Jan 30",
    teamSize: 1,
    Participation: "1 team per department (1 student per year)",
    imageUrl:
      "https://img.freepik.com/premium-photo/colorful-microphone-black-background-generative-ai_918839-1265.jpg?w=900",
  },
  "The Great Faculty Sing-Off": {
    start: "2:00 PM",
    end: "4:00 PM",
    location: "Instru Audi",
    category: "Fun (GATHERING HUB)",
    day: "Jan 29",
    teamSize: 1,
    Participation: "1 team from department", 
    imageUrl:
      "https://thumbs.dreamstime.com/b/vivid-illustration-microphone-musical-elements-ai-generated-vivid-illustration-microphone-musical-elements-335216799.jpg?w=992",
  },
  "CUMMINS GOT LATENT": {
    start: "12:00 PM",
    end: "2:30 PM",
    location: "Stage 2",
    category: "Fun (GATHERING HUB)",
    day: "Jan 30",
    teamSize: 1,
    Participation: "1 member per class",
    imageUrl:
      "https://www.rawpixel.com/image/14056921/magic-magician-light-night",
  },
  "Shantit Kranti": {
    start: "3:00 PM",
    end: "5:00 PM",
    location: "KB Joshi Audi",
    category: "Fun (GATHERING HUB)",
    day: "Jan 29",
    teamSize: 6,
    Participation: "Inter-Departmental (One Team per department)",
    imageUrl:
      "https://t4.ftcdn.net/jpg/10/85/32/61/240_F_1085326141_29OFQptRbrrVt2mLuLRsiEzwtJmhAFb2.jpg",
  },
  "KALAKAR KATTA": {
    start: "11:00 AM",
    end: "5:00 PM",
    location: "Instrumentation Quadrangle",
    category: "Fun (GATHERING HUB)",
    day: "Jan 31",
    teamSize: "-",
    Participation: "-",
    imageUrl:
      "https://img.freepik.com/premium-photo/vibrant-colors-celebrate-beauty-creativity-painted-women-portrait-generated-by-artificial-intelligence_24640-123697.jpg?w=900",
  },
  "ESCAPE ROOM": {
    start: "11:00 AM",
    end: "1:00 PM",
    location: "Entire IT/Mech Building",
    category: "Fun (GATHERING HUB)",
    day: "Jan 31",
    teamSize: 4,
    Participation: "1 team from each class (only first 20 teams: first come- first serve)",
    imageUrl:
      "https://img.freepik.com/premium-photo/inclusive-pridethemed-escape-room-experience-generative-ai_883586-207229.jpg",
  },
  "TALAASH": {
    start: "10:30 AM",
    end: "12:30 PM",
    location: "All Campus",
    category: "Fun (GATHERING HUB)",
    day: "Feb 1",
    teamSize: 4,
    Participation: "1 team from each class",
    imageUrl:
      "https://riddlepedia.com/wp-content/uploads/2024/02/prom-scavenger-hunt-riddles.png",
  },
  "HAUTEAURA": {
    start: "6:00 PM",
    end: "9:00 PM",
    location: "Samstha Ground",
    category: "Runway (HAUTE ROUTE)",
    day: "Jan 29",
    teamSize: "18 - 21 students",
    Participation: "1 team from each FINAL YEAR class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Miss Cummins-Beauty Contest": {
    start: "2:00 PM",
    end: "4:30 PM",
    location: "Main Building Quadrangle",
    category: "Runway (HAUTE ROUTE)",
    day: "Jan 31",
    teamSize: "18 - 21 students (14-17 performers + 4 helpers)",
    Participation: "2 participant from each class",
    imageUrl:
      "https://img.freepik.com/premium-photo/contestants-stunning-evening-gowns-walking-runway-beauty-pageant_1284400-6615.jpg",
  },
  "Eco Vogue": {
    start: "1:00 PM",
    end: "3:00 PM",
    location: "Main Building Quadrangle",
    category: "Runway (HAUTE ROUTE)",
    day: "Jan 30",
    teamSize: 3,
    Participation: "1 team per class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Goonj 3.0": {
    start: "2:30 PM",
    end: "4:30 PM",
    location: "Stage 2",
    category: "Literary (WORDSMITH’S CORNER)",
    day: "Jan 29",
    teamSize: 1,
    Participation: "1 team from each class", 
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Lights, Quiz, Action!": {
    start: "2:00 PM",
    end: "4:00 PM",
    location: "KB Joshi Audi",
    category: "Literary (WORDSMITH’S CORNER)",
    day: "Jan 31",
    teamSize: 5,
    Participation: "1 team from each class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Spellbound": {
    start: "12:30 PM",
    end: "2:30 PM",
    location: "KB Joshi Audi",
    category: "Literary (WORDSMITH’S CORNER)",
    day: "Jan 29",
    teamSize: "2-3 members",
    Participation: "One team per class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Mic in the Open": {
    start: "3:00 PM",
    end: "5:00 PM",
    location: "Stage 2",
    category: "Literary (WORDSMITH’S CORNER)",
    day: "Jan 31",
    teamSize: 1,
    Participation: "1 participant per class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Taste Wars": {
    start: "10:00 AM",
    end: "2:00 PM",
    location: "Between Main & IT Building",
    category: "Literary (WORDSMITH’S CORNER)",
    day: "Jan 31",
    teamSize: "2 members (minimum) / 4 members (maximum)",
    Participation: "1 team per class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Kicks and Dyes Showdown": {
    start: "12:30 PM",
    end: "2:30 PM",
    location: "Instru Quadrangle",
    category: "Artistry (ARTISCOPE)",
    day: "Jan 29",
    teamSize: "1 or 2",
    Participation: "1 team per class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Hands On Henna": {
    start: "3:30 PM",
    end: "5:30 PM",
    location: "Mech Circle",
    category: "Artistry (ARTISCOPE)",
    day: "Jan 29",
    teamSize: "1 (Self- artist)/ 2 (1 artist + 1 model)",
    Participation: "1 team per class",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },

  // have to add events of Artistry (ARTISCOPE) category

  "The Drama Premiere League": {
    start: "1:00 PM",
    end: "4:30 PM",
    location: "Main Building Quadrangle",
    category: "Drama (THEATRICAL TAPESTRY)",
    day: "Feb 1",
    teamSize: "5 (minimum)/ 10 (maximum) + 3 volunteers (if needed)",
    Participation: "20 teams (FCFS)",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  "Nazariya": {
    start: "12:00 PM",
    end: "4:00 PM",
    location: "Stage 2",
    category: "Drama (THEATRICAL TAPESTRY)",
    day: "Feb 1",
    teamSize: "8-20 (Including all music accompanists)",
    Participation: "-",
    imageUrl:
      "https://media.istockphoto.com/id/937674774/vector/abstract-modern-music-poster-template.jpg?s=612x612&w=0&k=20&c=8WTeLpUCJRWadbiQ_9cVSfjSa2x-CVOSG5M5FbYmJ9w=",
  },
  
};

export default event_details;